...
/* Preț pe complexitate: text/grafică mică pe față = 119; print mare pe
   spate SAU grafică detaliată (animale) pe față = 149. */
const TEE_SMALL = 119; // față simplă (text/grafică mică)
const TEE_BIG = 149;   // spate mare sau grafică detaliată pe față
const TEE_PRICE = TEE_BIG;
const BLANK = {
...
/* un tricou dintr-o singură culoare, cu designul pe `side` (front|back).
   `price` opțional; implicit 149 pt spate, 119 pt față. */
function tee(id, name, color, side, file, price) {
  const design = `/catalog/${file}`;
  return {
    id, collection: "streetwear", product: "tee", hero: side,
    name: { ro: name, en: name },
    price: price ?? (side === "back" ? TEE_BIG : TEE_SMALL),
    colors: [color],
...
export const CATALOG = [
  /* SET CUPLU — 2 tricouri (Beauty & Monster), 199 lei ambele. Imagine unică
     landscape, fără toggle față/spate. */
  {
    id: "couple-beauty-monster", collection: "streetwear", product: "tee", hero: "front",
    name: { ro: "SET CUPLU — Beauty & Monster", en: "COUPLE SET — Beauty & Monster" }, price: 199,
    colors: ["white"], pair: true, // set de 2 tricouri → 2 selectoare de mărime
    img: { white: "/catalog/tee-couple-beauty-monster.jpg" },
  },

  /* ALBE — print pe față (noile intrări sunt primele în grilă) */
  tee("dont-like-me",     "DON'T LIKE ME",     "white", "front", "tee-dont-like-me.jpg",     TEE_BIG),
  tee("no-means-no",      "NO MEANS NO",       "white", "front", "tee-no-means-no.jpg",      TEE_BIG),
  tee("go-get-that-dream","GO GET THAT DREAM", "white", "front", "tee-go-get-that-dream.jpg", TEE_BIG),
  ...
  /* ALBE — print pe spate */
  tee("more-expensive",  "MORE EXPENSIVE",     "white", "back", "tee-more-expensive-back.jpg"),
  tee("twenty-twenty",   "TWENTY-TWENTY",      "white", "back", "tee-twenty-twenty-back.jpg"),
  tee("every-religion",  "EVERY RELIGION",     "white", "back", "tee-every-religion-back.jpg"),
  tee("the-eyes-chico",  "THE EYES, CHICO",    "white", "back", "tee-the-eyes-chico-back.jpg"),
  tee("tequila-did",     "TEQUILA DID",        "white", "back", "tee-tequila-did-back.jpg"),
  tee("gin",             "GIN",                "white", "back", "tee-gin-back.jpg"),
